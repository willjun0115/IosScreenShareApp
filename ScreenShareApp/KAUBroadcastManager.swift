...
class KAUBroadcastManager: ObservableObject {
    ...
    func startStreaming(source: captureSource, roomID: String, mode: RTCClientMode = .broadcasterAsOfferer, preferredCodec: String = "H264") {
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            guard let self = self else { return }
            
            startConnection(roomID: roomID, mode: mode, preferredCodec: preferredCodec)
            ...
            case .screen:
                ...
                if #available(macOS 12.3, iOS 18.0, macCatalyst 18.2, *) {
                    ...
                    if isMac {
                        NSLog("✅ [BroadcastManager] Start receiving frames by ScreenCaptureKit.")
                        KAUReceiver.shared.stopReceiving()
                        self.macCapturer.startCapture(rtcManager: rtc)
                    } else {
                        NSLog("✅ [BroadcastManager] Start receiving frames by mmap.")
                        KAUReceiver.shared.startReceiving(rtcManager: rtc)
                    }
                } else {
                    NSLog("⚠️ [BroadcastManager] ScreenCaptureKit is not supported.")
                    KAUReceiver.shared.startReceiving(rtcManager: rtc)
                }
                #else
                ...
                #endif
            case .camera:
                NSLog("✅ [BroadcastManager] Start receiving frames by camera.")
                KAUReceiver.shared.stopReceiving()
                #if canImport(ScreenCaptureKit)
                if #available(macOS 12.3, iOS 18.0, macCatalyst 18.2, *) {
    ...
    func startConnection(roomID: String, mode: RTCClientMode = .broadcasterAsOfferer, preferredCodec: String = "H264") {
        ...
        rtcManager = WebRTCManager(socket: socket!, mode: mode, preferredCodec: preferredCodec)
        ...
        socket?.on("new-parent") { [weak self] data, _ in
            if let parentId = data[0] as? String {
                NSLog("✅ [Signaling] 새 부모 할당됨: \(parentId). Sending offer...")
        ...
        socket?.on("new-child") { [weak self] data, _ in
            if let childId = data[0] as? String {
                NSLog("✅ [Signaling] 새 시청자 접속: \(childId). Sending offer...")
        ...
        socket?.on("candidate") { [weak self] data, _ in
            // NSLog("📥 [Signaling] Received candidate: \(data)")
            if let dict = data[0] as? [String: Any],
               let fromId = dict["from"] as? String,
               ...
               let candidateDict = candData["candidate"] as? [String: Any] {
                ...
                NSLog("❌ [Signaling] Failed to parse candidate.")
            }
        }
